...
ddf.drop(columns=['관서명'], inplace=True)
ddf = ddf.pivot_table(index='구별', aggfunc='sum')
ddf.drop(['구 없음'], inplace=True)

print(ddf)
# Q5 ------------------------------------------------------------------------

ddf['강간검거율'] = (ddf['강간(검거)'] / ddf['강간(발생)']) * 100
ddf['강도검거율'] = (ddf['강도(검거)'] / ddf['강도(발생)']) * 100
ddf['살인검거율'] = (ddf['살인(검거)'] / ddf['살인(발생)']) * 100
ddf['절도검거율'] = (ddf['절도(검거)'] / ddf['절도(발생)']) * 100
ddf['폭력검거율'] = (ddf['폭력(검거)'] / ddf['폭력(발생)']) * 100 

ddf['검거율'] = (ddf['소계(검거)'] / ddf['소계(발생)']) * 100 
...
print(ddf)
# Q7 ------------------------------------------------------------------------
dic = {'강간(발생)':'강간', '강도(발생)':'강도', '살인(발생)':'살인', '절도(발생)':'절도', '폭력(발생)':'폭력'}
ddf.rename(dic, inplace=True, axis=1)
